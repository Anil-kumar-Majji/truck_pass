...
export const ConsentAndAgreementPage: React.FC<ConsentAndAgreementPageProps> = ({ getStartedBtn, setGetStartedBtn }) => {
    ...
    return (
        <div className={`flex flex-col bg-[#FFFFFF] pt-5 pb-9 w-full px-6 rounded-br-2xl rounded-tr-2xl justify-between`}>
            ...
            <button disabled={!checkBox} onClick={() => setGetStartedBtn(true)}
                className={`${checkBox ? "bg-[#006DE7] cursor-pointer" : "bg-[#B0B0B0] cursor-default"} w-[54%] text-sm font-[600] place-self-end align-bottom py-2 text-center rounded-[5px] text-[#FFFFFF] no-underline cursor-pointer`}>
                Get started
            </button>
        </div>
    )
